...
def test_find_merkle_path():
    ...
    path_1 = tree.find_merkle_path(tree_1, hash_1)
    assert len(path_1) == 1
    assert path_1[0] == hash_1

    path_2 = tree.find_merkle_path(tree_1, hash_2)
    assert path_2 is None

    path_3 = tree.find_merkle_path(tree_12, hash_1)
    assert len(path_3) == 3
    assert path_3[0] == hash_12
    assert path_3[1] == hash_2
    assert path_3[2] == hash_1

    path_4 = tree.find_merkle_path(tree_12, hash_2)
    assert len(path_3) == 3
    assert path_4[0] == hash_12
    assert path_4[1] == hash_1
    assert path_4[2] == hash_2

    path_5 = tree.find_merkle_path(tree_123, hash_3)
    assert len(path_5) == 3
    assert path_5[0] == hash_123
    assert path_5[1] == hash_12
    assert path_5[2] == hash_3

    path_6 = tree.find_merkle_path(tree_123, hash_4)
    assert path_6 is None

    path_7 = tree.find_merkle_path(tree_1234, hash_1)
    assert len(path_7) == 4
    ...
    path_8 = tree.find_merkle_path(tree_1234, hash_4)
    assert len(path_8) == 4
    ...
    assert tree.validate_merkle_path(path_1)
    assert not tree.validate_merkle_path(path_2)
    assert tree.validate_merkle_path(path_3)
    assert tree.validate_merkle_path(path_4)
    assert tree.validate_merkle_path(path_5)
    assert not tree.validate_merkle_path(path_6)
    assert tree.validate_merkle_path(path_7)
    assert tree.validate_merkle_path(path_8)
